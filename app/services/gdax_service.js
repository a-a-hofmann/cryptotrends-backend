...
const { HISTORICAL_COLLECTION, PAIRS } = require('../utils/Constants');
...
module.exports = {
    async fetchHistoricalData(db) {
        ...
        for (let i = 0; i < PAIRS.length; i++) {
            ...
            console.log(chalk.cyan(`GDAX SERVICE::Fetching symbol: ${symbol}.`));
            
            // Need to throttle requests. GDAX allows up to 3 per second. So might as well await for the data to arrive.
            [err, historicalDataUpTo300DaysAgo] = await on(fetchDailyRateForSymbol(symbol, threeHundredDaysAgo.toISOString(), today.toISOString()));
            if (err) {
